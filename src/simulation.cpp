...
void Simulation::compute_next_step()
{
    ...
    for (int i = 0; i < Config::GRID_SIZE; i++)
    {
        for (int j = 0; j < Config::GRID_SIZE; j++)
        {
            x0 = Config::GRID_SIZE * (x - Config::time_step * old_state.velocity_x[i + Config::GRID_SIZE * j]) - 0.5f;
            y0 = Config::GRID_SIZE * (y - Config::time_step * old_state.velocity_y[i + Config::GRID_SIZE * j]) - 0.5f;
            ...
            cur_state.velocity_x[i + Config::GRID_SIZE * j] = (1 - s) * ((1 - t) * old_state.velocity_x[i0 + Config::GRID_SIZE * j0] + t * old_state.velocity_x[i0 + Config::GRID_SIZE * j1]) + s * ((1 - t) * old_state.velocity_x[i1 + Config::GRID_SIZE * j0] + t * old_state.velocity_x[i1 + Config::GRID_SIZE * j1]);
            cur_state.velocity_y[i + Config::GRID_SIZE * j] = (1 - s) * ((1 - t) * old_state.velocity_y[i0 + Config::GRID_SIZE * j0] + t * old_state.velocity_y[i0 + Config::GRID_SIZE * j1]) + s * ((1 - t) * old_state.velocity_y[i1 + Config::GRID_SIZE * j0] + t * old_state.velocity_y[i1 + Config::GRID_SIZE * j1]);
    ...
    for (int i = 0; i < Config::GRID_SIZE; i++)
    {
        for (int j = 0; j < Config::GRID_SIZE; j++)
        {
            old_state.velocity_x[i + (Config::GRID_SIZE + 2) * j] = cur_state.velocity_x[i + Config::GRID_SIZE * j];
            old_state.velocity_y[i + (Config::GRID_SIZE + 2) * j] = cur_state.velocity_y[i + Config::GRID_SIZE * j];
        };
    };
    ...
    for (int i = 0; i <= Config::GRID_SIZE; i += 2)
    {
        ...
        for (int j = 0; j < Config::GRID_SIZE; j++)
        {
            ...
            old_state.velocity_x[i + (Config::GRID_SIZE + 2) * j] = f * ((1 - x * x / r) * U[0] - x * y / r * V[0]);
            old_state.velocity_x[i + 1 + (Config::GRID_SIZE + 2) * j] = f * ((1 - x * x / r) * U[1] - x * y / r * V[1]);
            old_state.velocity_y[i + (Config::GRID_SIZE + 2) * j] = f * (-y * x / r * U[0] + (1 - y * y / r) * V[0]);
            old_state.velocity_y[i + 1 + (Config::GRID_SIZE + 2) * j] = f * (-y * x / r * U[1] + (1 - y * y / r) * V[1]);
        };
    };
    ...
    for (int i = 0; i < Config::GRID_SIZE; i++)
    {
        for (int j = 0; j < Config::GRID_SIZE; j++)
        {
            cur_state.velocity_x[i + Config::GRID_SIZE * j] = f * old_state.velocity_x[i + (Config::GRID_SIZE + 2) * j];
            cur_state.velocity_y[i + Config::GRID_SIZE * j] = f * old_state.velocity_y[i + (Config::GRID_SIZE + 2) * j];
